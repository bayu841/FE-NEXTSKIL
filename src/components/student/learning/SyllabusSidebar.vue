...
<template>
  <div
    class="w-full md:w-[350px] bg-white border-l border-gray-200 flex flex-col h-full shrink-0"
  >
    <div class="p-5 border-b border-gray-100">
      <h3 class="font-black text-gray-900 text-lg">Silabus Kursus</h3>
      <p class="text-sm text-gray-500 font-medium">
        {{ finishedLessons.length }} dari 6 Materi diselesaikan ({{
          Math.round((finishedLessons.length / 6) * 100)
        }}%)
      </p>
      <div class="h-1.5 bg-gray-100 rounded-full mt-3 overflow-hidden">
        <div
          class="h-full bg-emerald-500 rounded-full transition-all duration-500"
          :style="{ width: (finishedLessons.length / 6) * 100 + '%' }"
        ></div>
      </div>
    </div>
        ...
        <div v-show="mod.isOpen" class="pt-2 pb-1 space-y-1">
          ...
          >
            ...
            <div class="mt-0.5 mr-3 flex-shrink-0">
              <CheckCircle2
                v-if="finishedLessons.includes(lesson.id)"
                class="w-4 h-4 text-emerald-500"
              />
              <Lock
                v-else-if="checkLessonLocked(lesson.id)"
                class="w-4 h-4 text-gray-300"
              />
              <div
                v-else
                class="w-4 h-4 rounded-full border-2 border-indigo-200 animate-pulse"
              ></div>
            </div>
            ...
            <div class="flex-1">
              ...
              <div
                class="flex items-center gap-2 mt-1.5 text-[11px] font-medium text-gray-400"
              >
                <PlayCircle
                ...
                />
                <Download
                  v-else-if="lesson.type === 'file'"
                  class="w-3.5 h-3.5"
                />
                <ClipboardList
                  v-else-if="lesson.type === 'task'"
                  class="w-3.5 h-3.5"
                />
                <span>{{
                  lesson.type === "video"
                ...
                <span
                  v-if="checkLessonLocked(lesson.id)"
                  class="ml-auto text-[9px] font-black text-gray-300 uppercase italic"
                  >Locked</span
                >
              </div>
            </div>
